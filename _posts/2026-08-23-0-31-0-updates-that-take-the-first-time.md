...
Some of you updated, watched the device come back, and found the old version
...
Updating to this version is enough and needs no cable. The restart that installs
it is still done by the old firmware, so if the device comes back on the old
version, run the update once more. From this version on it stops happening.
Found in a serial log by
...
was watching for, so it was read as a broken connection and the upload thrown
away. Live video can starve a socket for half a minute, so updating with the
console open could fail every time. A silence is treated as a silence now.
