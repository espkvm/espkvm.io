...
which is most of what a BIOS asks of you. It is a standing preference, not a
place you go: with it on, the view follows the machine. Characters at the boot
menu, the picture the moment a desktop paints, characters again at the next
restart. Both directions need two readings to agree, so a screen near the edge
of being readable does not flicker between them. The readings come along the
same socket the picture used and carry only what changed, so walking a boot menu
...
with no grid to cut. It does have a shape a few hundred samples can see: nearly
all one colour, and it stays. The device reports how long the picture has been
flat and publishes it to Home Assistant. What that means is yours to decide.
...
**A viewing token.** Home Assistant's camera integrations take a URL and little
else, so putting a target's screen on a dashboard meant turning the login off. A
token lives in Settings, Security, and there is none until you make one. It
opens the MJPEG stream, a single frame and the capture figures, and it cannot
press a key or cut the power. Only its hash is kept, so you see it once.
