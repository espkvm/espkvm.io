...
Behind those answers is a machine, not a video of one: a state machine with
stages, a clock, and an 80x25 character screen with the same geometry the
firmware reports for a real UEFI console.

The page opens on a machine mid-POST and lands on "No boot device found". The
screen tells you to pick an image under Media. Choosing one writes the same
setting that mounts a disc on a real device. Reset boots it. The prompt takes
the keys you type. Do nothing for fifteen seconds and the demo picks an image
itself, and the first thing you do stops that for good.
...
repeated a letter, which is what fast typing looks like.
