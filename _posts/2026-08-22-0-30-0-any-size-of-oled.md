...
All of them work now. You pick controller and size from one list: the same chip
drives every size and reports none of it, so this is the one thing that cannot
be detected. Devices already set up keep the panel they had.

A shorter panel shows fewer lines and drops the least useful first: the address
stays, the uptime goes. A line too long for the glass steps along a character at
a time instead of being cut off, and the screen waits for it to finish before
moving on. A clipped address looks like a real one.
