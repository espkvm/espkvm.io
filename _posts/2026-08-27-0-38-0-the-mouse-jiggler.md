...
so the thing you wanted to look at is behind a password you have to type over a
KVM.
...
It runs on the device, not in the browser, on purpose: it exists for a machine
nobody is sitting in front of, and a jiggler in a browser tab goes away with the
tab. It stands aside while you are using the mouse yourself, and does nothing
when no target is attached. Off by default.
